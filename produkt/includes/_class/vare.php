...
class Vare extends dbase
{
    ...
    {
        $mysqli = parent::connect();
        $soktekst = mysqli_real_escape_string($mysqli, $soktekst);
        $varer = "SELECT  
                            vare.idvare, 
        ...
        echo '<div id="sokResultat">';
        if($num < 1)
        {
            echo '<h2>Kunne ikke finne noen varer.</h2>';
        }
            ...
            echo '<tr class="katRad">';
                 for($i=0; $i < 3; $i++)
                 {
                     if($teller < $num)
                     {
                         ...
                         echo '<td class="katKol">';
                             echo '
                                    <a href="index.php?idvare='.$valg->idvare.'"><img src="bilder/opplastet/'.$valg->bildeurl.'" width = "230" /></a>
                                    <h1><a href="index.php?idvare='.$valg->idvare.'">'.$valg->tittel.'</a></h1> 
                                    <p>'.$this->parseTekst($valg->tekst).'<br/><br/>
                                        <a href="index.php?idvare='.$valg->idvare.'">Les mer</a></p>
                                    <p>'.$this->paLager($valg->antall).'</p> 
                                    <h2>'.$valg->pris.',-<a href="?kat='.$valg->katid.'&action=add&id='.$valg->idvare.'">
                                        <img src="includes/images/kjop.jpg" style="float:right" /></a></h2>';
                         echo '</td>';
                         echo "\n\t\t\t\t";
                     }
